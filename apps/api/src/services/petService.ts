...
export interface CreatePetData {
  ...
  sex: string; neutered?: boolean; weightGoalKg?: number;
}
export interface UpdatePetData {
  name?: string; species?: string; breed?: string; birthDate?: Date;
  sex?: string; neutered?: boolean; weightGoalKg?: number; photoUrl?: string;
}
...
async function getUserHouseholdIds(userId: string): Promise<string[]> {
  ...
  return memberships.map((m: { householdId: string }) => m.householdId);
}

export async function listPets(userId: string): Promise<Pet[]> {
  const householdIds = await getUserHouseholdIds(userId);
  return prisma.pet.findMany({ where: { householdId: { in: householdIds } }, orderBy: { createdAt: 'asc' } });
}

export async function getPet(id: string, userId: string): Promise<Pet | null> {
  const householdIds = await getUserHouseholdIds(userId);
  return prisma.pet.findFirst({ where: { id, householdId: { in: householdIds } } });
}

export async function createPet(userId: string, data: CreatePetData): Promise<Pet> {
  const membership = await prisma.householdMember.findFirst({ where: { userId }, orderBy: { joinedAt: 'asc' } });
  if (!membership) throw new Error('User has no household');
  return prisma.pet.create({ data: { ...data, householdId: membership.householdId } });
}
...
export async function updatePetPhoto(id: string, userId: string, photoUrl: string): Promise<Pet | null> {
  const pet = await getPet(id, userId);
  if (!pet) return null;
  return prisma.pet.update({ where: { id }, data: { photoUrl } });
}
